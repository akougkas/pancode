...
function renderModeLine(data: FooterData, width: number, c: TuiColorizer): string {
  const sep = ` ${DASH}${DASH} `;
  const isGodMode = data.modeName === "Admin";
  ...
  // Display "GOD MODE" instead of "Admin" when in admin mode.
  const modeLabel = isGodMode ? "GOD MODE" : data.modeName;

  // Plain text for width calculation
  const plainParts = [modeLabel, data.safety, modelShort, `Reasoning: ${data.reasoning}`];
...
export function renderFooterLines(data: FooterData, width: number, c: TuiColorizer): string[] {
  ...
  if (width < 60) {
    ...
    const narrowLabel = data.modeName === "Admin" ? "GOD MODE" : data.modeName;
    return [truncateToWidth(c.mode(`[${narrowLabel}]`) + activity + c.dim(` ${data.contextPercent}%`), width)];
  }
