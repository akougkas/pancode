...
export const MODE_CAPTURE_FRONTIER: Fragment = {
  id: "orch.mode.capture.frontier",
  version: 1,
  roles: ["orchestrator"],
  tiers: ["frontier"],
  modes: ["capture"],
  ...
  estimatedTokens: 50,
  text: [
    "MODE:CAPTURE. Log ideas and requirements. No dispatch, no code changes.",
    "FLOW:Listen to user -> create tasks with task_write -> organize with task_update.",
    "ANTI:Do not call dispatch_agent, batch_dispatch, or dispatch_chain.",
    "ANTI:Do not use write or edit tools. Read-only observation.",
  ].join("\n"),
};

export const MODE_CAPTURE_COMPACT: Fragment = {
  id: "orch.mode.capture.compact",
  version: 1,
  roles: ["orchestrator"],
  tiers: ["mid", "small"],
  modes: ["capture"],
  category: "mode",
  estimatedTokens: 30,
  text: "MODE:CAPTURE. Log tasks with task_write. No dispatch. No code changes.",
};
...
export const MODE_ASK_FRONTIER: Fragment = {
  id: "orch.mode.ask.frontier",
  version: 1,
  roles: ["orchestrator"],
  tiers: ["frontier"],
  modes: ["ask"],
  ...
  estimatedTokens: 50,
  text: [
    "MODE:ASK. Answer questions and explore the codebase. Readonly operations only.",
    "FLOW:Use shadow_explore for research. Read files directly for specific questions.",
    "FLOW:Answer concisely. Provide file paths and line numbers when referencing code.",
    "ANTI:Do not dispatch workers. Do not modify files.",
  ].join("\n"),
};

export const MODE_ASK_COMPACT: Fragment = {
  id: "orch.mode.ask.compact",
  version: 1,
  roles: ["orchestrator"],
  tiers: ["mid", "small"],
  modes: ["ask"],
  category: "mode",
  estimatedTokens: 25,
  text: "MODE:ASK. Answer questions. Use shadow_explore for research. No dispatch. No file changes.",
};
...
export const SAFETY_AWARENESS_FRONTIER: Fragment = {
  ...
  text: [
    ...
    "Attempting mutable dispatch in ask/review/plan/capture mode returns an error.",
...
export const TOOL_SHADOW_FRONTIER: Fragment = {
  ...
  text: [
    ...
    "shadow_explore dispatches 1-4 concurrent scouts to search the codebase.",
    "Each scout is limited to 3-5 tool calls. Results are structured findings with file paths.",
    "",
    "WHEN TO USE:Before dispatch decisions. Open-ended questions about project structure.",
    "  Multiple queries run in parallel. Use 2-4 queries for broad reconnaissance.",
    "WHEN NOT TO USE:Simple single-file lookups (use read/grep directly).",
...
export const TOOL_SHADOW_COMPACT: Fragment = {
  ...
  estimatedTokens: 40,
  text: [
    "shadow_explore:1-4 parallel scouts, 3-5 calls each. Use before dispatch for recon.",
    "Do not use for simple single-file reads (use read/grep directly).",
...
export const ALL_FRAGMENTS: readonly Fragment[] = [
  ...
  ORCH_IDENTITY_SMALL,
  // Orchestrator modes
  MODE_CAPTURE_FRONTIER,
  MODE_CAPTURE_COMPACT,
  MODE_PLAN_FRONTIER,
  MODE_PLAN_COMPACT,
  MODE_BUILD_FRONTIER,
  MODE_BUILD_COMPACT,
